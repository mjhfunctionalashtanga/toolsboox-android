// Fully-qualified URLs (reader://app/vendor/pdfjs/…). pdf.js resolves these with
// `new URL(path, location)` internally — a path-absolute "/vendor/…" fails for our
// non-special reader:// scheme ("does not resolve to valid URL"), so build the
// absolute href from this module's own URL.
const pdfjsPath = path => new URL(`./vendor/pdfjs/${path}`, import.meta.url).href

// Vendored non-minified build served over the reader:// scheme (no bundler /
// import map here, so reference the real files directly).
