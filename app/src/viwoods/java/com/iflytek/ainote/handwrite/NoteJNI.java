...
public class NoteJNI {
    ...
    static {
        ...
        // Load libpaintworker.so from THIS class's static initializer. The canonical JNI idiom:
        // JNI_OnLoad's FindClass("com/iflytek/ainote/handwrite/NoteJNI") anchors on the classloader
        // that initiated the load, so loading here (NoteJNI == the class being registered, in the
        // app classloader) is what lets RegisterNatives resolve this class. Loading from anywhere
        // else (framework/boot classloader) fails with "NoClassDefFound … NoteJNI". Must run BEFORE
        // ENoteSetting.initWriting() so this load — not the framework's — brings the library in.
        try {
            System.loadLibrary("paintworker");
            Log.d(TAG, "NoteJNI loaded libpaintworker from app classloader");
        } catch (Throwable t) {
            Log.w(TAG, "NoteJNI libpaintworker load failed", t);
        }
    }
